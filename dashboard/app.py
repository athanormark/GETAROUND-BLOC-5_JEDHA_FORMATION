...
<style>
    ...
    .section-title {
        color: #5ce1e6; border-bottom: 1px solid #2a2d3a;
    ...
    .footer {
        text-align: center; color: #556; font-size: 0.85rem;
...
# Template Plotly dark (coherent avec le theme)
PLOTLY_LAYOUT = dict(
...
# --- Barre laterale : parametres de simulation ---
...
# Histogramme des retards (clippe pour lisibilite)
fig = px.histogram(
...
fig.update_layout(
    title="Impact du seuil sur les cas resolus et les locations bloquees",
    **PLOTLY_LAYOUT
)
fig.update_xaxes(title_text="Seuil (minutes)", gridcolor="#2a2d3a")
fig.update_yaxes(
    title_text="% cas resolus", secondary_y=False, gridcolor="#2a2d3a"
)
fig.update_yaxes(
    title_text="% locations bloquees", secondary_y=True, gridcolor="#2a2d3a"
)
st.plotly_chart(fig, use_container_width=True)
